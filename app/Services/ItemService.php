...
class ItemService
{
    ...
    public function simpleList(PaginateRequest $request)
    {
        try {
            ...
            return Item::with('media', 'category', 'offer', 'translations')->where(function ($query) use ($requests) {
                foreach ($requests as $key => $request) {
                    if (in_array($key, $this->itemFilter)) {
                        if ($key == "except") {
                            $explodes = explode('|', $request);
    ...
    {
        try {
            return $item->load('media', 'category', 'tax', 'offer', 'addons', 'variations', 'extras', 'translations');
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception(QueryExceptionLibrary::message($exception), 422);
        }
    ...
    {
        try {
            return Item::with('media', 'category', 'offer', 'translations')->where(['is_featured' => Ask::YES, 'status' => Status::ACTIVE])->inRandomOrder()->limit(8)->get();
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception(QueryExceptionLibrary::message($exception), 422);
        }
    ...
    {
        try {
            return Item::with('media', 'category', 'offer', 'translations')->withCount('orders')->where(['status' => Status::ACTIVE])->orderBy('orders_count', 'desc')->limit(6)->get();
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception(QueryExceptionLibrary::message($exception), 422);
        }
    ...
    {
        return $item->load('media', 'category', 'tax', 'offer', 'addons', 'variations', 'variations.itemAttribute.translations', 'extras', 'translations');
    }
}
