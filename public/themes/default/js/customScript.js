...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const startDrag = (e) => {
    const target = e.target.closest('.ai-move');
    if (!target) return;
    ...
    // Setting these so dragging does not have weird jumping physics
    ...
    // To remove the top-1/2 -translate-y-1/2 effect during drag, but preserve actual coordinates
    aibox.style.transform = 'none';
    aibox.style.right = 'auto';
    aibox.style.bottom = 'auto';
    aibox.style.left = `${rect.left}px`;
    aibox.style.top = `${rect.top}px`;
  };
  ...
  const onDrag = (e) => {
    ...
    if (e.type === 'touchmove' && hasDragged && e.cancelable) {
      e.preventDefault();
    }
  ...
  // Capture clicks globally so we can stop propagation if a drag occurred.
  document.addEventListener('click', (e) => {
    const target = e.target.closest('.ai-move');
    if (target && hasDragged) {
      e.preventDefault();
