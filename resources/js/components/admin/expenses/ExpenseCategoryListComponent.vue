    ...
    <div class="col-12">
        <div class="db-card">
            <div class="db-card-header">
                <h3 class="db-card-title">{{ $t('menu.expense_categories') }}</h3>
            ...
            <div class="db-card-body">
                ...
                <div class="db-table-responsive">
                    <table class="db-table stripe">
                        ...
                        <tbody class="db-table-body" v-if="categories.length > 0">
                            <tr class="db-table-body-tr" v-for="item in categories" :key="item.id">
                ...
                <div class="flex items-center justify-between border-t border-gray-200 dark:border-gray-800 pt-4" v-if="categories.length > 0">
...
export default {
    ...
    methods: {
        ...
        list: function (page = 1) {
            ...
            this.$store.dispatch('expenseCategory/lists', this.props.search).then(() => {
        ...
        edit: function (item) {
            ...
            this.$store.dispatch('expenseCategory/edit', item.id);
            this.props.form = {
                name: item.name,
        ...
        destroy: function (id) {
            appService.destroyConfirmation().then((res) => {
                if (res.isConfirmed) {
                    this.loading.isActive = true;
                    this.$store.dispatch('expenseCategory/destroy', { id: id, search: this.props.search }).then(() => {
                        this.loading.isActive = false;
                        alertService.clearPromt();
                    }).catch((err) => {
                        this.loading.isActive = false;
                        alertService.error(err.response?.data?.message || 'Error deleting category');
                    });
                }
            }).catch();
        }
