<template>
    ...
    <div class="db-card-header border-none flex items-center justify-between">
        ...
        <button
            ...
            type="button"
            data-modal="#offerTranslationModal"
...
export default {
    ...
    computed: {
        ...
        completedCount() {
            return this.languages.filter(lang => this.filledCount(lang.code) === 1).length;
        },
        overallProgress() {
    ...
    methods: {
        ...
        openForm(code = null) {
            ...
            appService.modalShow('#offerTranslationModal');
        },
        closeForm() {
            appService.modalHide('#offerTranslationModal');
            this.initForm();
        },
        hasValue(value) {
        ...
        filledCount(code) {
            ...
            if (!t) return 0;
            return [t.name].filter(v => this.hasValue(v)).length;
        },
        ...
        statusLabel(code) {
            ...
            if (filled === 1) return this.$t('label.completed');
            return this.$t('label.pending');
        },
        statusClass(code) {
            const filled = this.filledCount(code);
            if (filled === 1) return 'db-table-badge text-green-600 bg-green-100';
            return 'db-table-badge text-red-600 bg-red-100';
        },
        save() {
            this.loading.isActive = true;
            this.$store.dispatch('offer/saveTranslations', {
                ...
                appService.modalHide('#offerTranslationModal');
                this.$store.dispatch('offer/show', this.$route.params.id);
            }).catch(err => {
                this.loading.isActive = false;
