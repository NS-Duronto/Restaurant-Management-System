            ...
            <div class="flex flex-wrap gap-y-5 items-end justify-between">
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <button type="button" v-print="printObj"
                        class="flex items-center justify-center gap-2 px-4 h-[38px] rounded shadow-db-card bg-primary hover:bg-primary/90 transition">
                        <i class="lab lab-printer-line lab-font-size-16 text-white"></i>
                        <span class="text-sm capitalize text-white font-medium">{{ $t('button.print_invoice') }}</span>
                    </button>
                </div>
            </div>
        </div>
                    ...
                    <div class="mb-3 pb-3 border-b last:mb-0 last:pb-0 last:border-b-0 border-gray-200 dark:border-gray-800"
                        ...
                        <ul v-if="item.item_extras.length > 0 || item.instruction !== ''"
                            ...
                            <li class="flex gap-1" v-if="item.item_extras.length > 0">
                                <h3 class="capitalize text-xs w-fit whitespace-nowrap font-medium">{{ $t('label.extras') }}:</h3>
                                <p class="text-xs">
                ...
                <div class="db-card p-1">
                    <ul class="flex flex-col gap-2 p-3 border-b border-dashed border-gray-200 dark:border-gray-800">
                        <li class="flex items-center justify-between text-gray-700 dark:text-gray-300">
                            ...
                            <span class="text-sm leading-6 font-semibold">{{ order.subtotal_currency_price }}</span>
                        </li>
                        <li class="flex items-center justify-between text-gray-700 dark:text-gray-300">
                            <span class="text-sm leading-6 capitalize">{{ $t('label.discount') }}</span>
                            <span class="text-sm leading-6 font-semibold">{{ order.discount_currency_price }}</span>
                        </li>
                    </ul>
                    <div class="flex items-center justify-between p-3">
                        <h4 class="text-sm leading-6 font-bold capitalize text-heading dark:text-gray-100">{{ $t('label.total') }}</h4>
